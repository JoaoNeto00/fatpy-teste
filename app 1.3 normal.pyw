...
class Fatpy(ttk.Frame):
    ...
    def gerar_fatura_energia(self):
        ...
        total_kw = ((df['Leitura Atual'] - df['Leitura Anterior']) * df['Fator']) + df['Fator Demanda']
        ...
        colunas_necessarias = (zip(df['Cliente'], df['CNPJ'], df['Local'], df['Leitura Atual'],
                   df['Leitura Anterior'], df['Fator'], df['Fator Demanda'], df['VALOR A COBRAR']))
        
        print('gerando faturas..')
        
        for i, (cliente, cnpj, local, lt_atual, lt_anterior, fator, ft_demanda, vl_a_cobrar) in enumerate(colunas_necessarias, start=0):
